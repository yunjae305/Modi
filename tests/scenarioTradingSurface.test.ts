...
test('시뮬레이션 화면은 종목 목록과 시장 시간 컨트롤을 사용한다', () => {
  ...
  assert.match(marketTimeControls, /4배속/);
  assert.equal(marketTimeControls.includes('3배속'), false);
  assert.equal(marketTimeControls.includes('0.5배속'), false);
...
test('시뮬레이션 데이터는 5거래일 구간으로 압축해 재생한다', () => {
  const useChartData = readFileSync('src/hooks/useChartData.ts', 'utf8');
  const simulationPage = readFileSync('src/pages/SimulationPage.tsx', 'utf8');
  const nextDayButton = readFileSync('src/components/trade/NextDayButton.tsx', 'utf8');

  assert.match(useChartData, /compressScenarioStocks/);
  assert.match(simulationPage, /구간/);
  assert.match(nextDayButton, /다음 구간/);
  assert.equal(nextDayButton.includes('다음 날'), false);
});
