...
import { sell } from '../_lib/trading.ts';
...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : '매도 주문을 처리하지 못했습니다.';
    if (message.includes('로그인')) {
      return fail(res, 401, message);
    }
    if (message.includes('수량') || message.includes('보유') || message.includes('종목')) {
